...
namespace RevitApiOnline.CreateFilterLegend
{
    public class ColorAndPattern
    {
        ...
        {
            Func<FillPatternElement, bool> func = (FillPatternElement element) =>
            {
                bool draftTarget = element.GetFillPattern().Target == FillPatternTarget.Drafting;
                bool isSolid = element.GetFillPattern().IsSolidFill;
                return draftTarget && !isSolid;
            };
